...
def liqing_evaluate():
    prediction_json = "/home/liu/ytx/road_damage/prediction_json"
    target_json = "/home/liu/ytx/road_damage/target_json"
    ...
    for key in match_prediction:
        precision[key] = round(match_prediction[key] / (prediction_sta[key] + 1e-7), 3)
        recall[key] = round(match_target[key] / (target_sta[key] + 1e-7), 3)
        F1_score[key] = round(2 * (precision[key] * recall[key]) / ((precision[key] + recall[key]) + 1e-7), 3)
        F2_score[key] = round(5 * (precision[key] * recall[key]) / ((4 * precision[key] + recall[key]) + 1e-7), 3)
    ...
    res = {"precision": precision,
           ...
           "F1": F1_score,
           "F2": F2_score,
           "AVG_F1": '%.3f' % (F1_sum / len(F1_score)),
           "AVG_F2": '%.3f' % (F2_sum / len(F2_score))}

    print("F1分数：{}".format(res["AVG_F1"]))
