...
import sys
sys.path.append('/home/liu/ytx/road_damage')
...
def test():
    for img_name in tqdm(file_list):
        ...
        bboxes, labels = show_result(img_path, result, CLASSES, score_thr=0.0)
...
if __name__ == "__main__":
    ...
    # TODO 根據配置文件修改
    config_path = '/home/liu/ytx/road_damage/outputs/faster_rcnn'
    config_file = config_path + '/faster_rcnn.py'
    pth_file = config_path + "/*.pth"
    all_pth = sorted(glob.glob(pth_file), key=os.path.getmtime)

    # 测试文件图片文件夹
    file_root = '/home/liu/ytx/road_damage/data/coco/val2017/'
    ...
    # 测试结果保存文件夹
    json_output_folder = "/home/liu/ytx/road_damage/prediction_json"

    # 打印日志信息
    logger = logging.getLogger("eval_log")
    logger.setLevel(level=logging.INFO)
    handler = logging.FileHandler(config_path + "/eval_log.txt")
